...
from stac_geoparquet import arrow
...
def new_stac_table(input_parquet_file: str, catalog, warehouse: str, table_name: str, table_name_space: str = "stac"):
    ...
    print(f"Creating table `{iceberg_table}` with {len(table_schema.columns)} columns...")
    table = catalog.create_table(
        identifier=iceberg_table, schema=table_schema, location=s3_data_location, properties=table_props()
    )
...
def main(input_parquet_file: str, config: dict):
