...
def load_config(config_file: str):
    ...
    if missing_keys:
        raise ValueError(f"Missing required keys in {config_file}: {', '.join(missing_keys)}")
...
def postgres_connection_string(
    postgres_host: str, postgres_port: str, postgres_user: str, postgres_password: str, postgres_db: str
):
    """Generate a PostgreSQL connection string."""
    return f"postgresql+psycopg://{postgres_user}:{postgres_password}" f"@{postgres_host}:{postgres_port}/{postgres_db}"
